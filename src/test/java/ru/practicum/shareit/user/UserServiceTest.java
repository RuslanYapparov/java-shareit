...
public class UserServiceTest {
    ...
    @ParameterizedTest
    @ValueSource(strings = {"valera@user.ru", "iliusha@user.ru"})
    ...
    @Test
    public void save_whenGetEmailWithoutDot_thenThrowException() {
        UserRestCommand userRestCommand = UserRestCommand.builder()
                .name("Gosha")
                .email("gosha@gosha")
                .build();
        BadRequestBodyException exception = assertThrows(BadRequestBodyException.class, () ->
                userService.save(userRestCommand));
        assertThat(exception.getMessage(), equalTo("Невозможно сохранить пользователя 'Gosha'. Причина - " +
                        "неправильный формат адреса электронной почты 'gosha@gosha': отсутствует точка в домене"));
    }
    ...
    public void update_whenGetRestCommandWithOneField_thenReturnUpdatedUserRestView() {
        ...
        assertThat(firstUser.getEmail(), equalTo("valera@user.ru"));
        ...
        firstUser = userService.getById(firstUser.getId());
        assertThat(firstUser.getName(), equalTo("Gosha"));
        assertThat(firstUser.getEmail(), equalTo("gosha@user.ru"));
        assertThat(firstUser.getCreated(), equalTo(created));
        assertThat(firstUser.getLastModified(), not(lastModified));
    }
